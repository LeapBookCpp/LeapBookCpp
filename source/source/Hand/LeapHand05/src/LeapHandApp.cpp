...
class LeapHandApp : public AppNative {
	...
	void draw()
  {
    ...
    for ( auto hand : frame.hands() ) {
      ...
      gl::lineWidth( 10 );
      gl::begin( GL_LINES );
      gl::vertex( toVec3f( hand.palmPosition() ) );
      gl::vertex( toVec3f( hand.palmPosition() ) + (toVec3f( hand.direction() ) * Vec3f( lineLength, lineLength, lineLength )) );
      gl::end();

      {
        // 法線を表示する
        setDiffuseColor( ci::ColorA( red, 1.0, 1.0 ) );
        gl::begin( GL_LINES );
        gl::vertex( toVec3f( hand.palmPosition() ) );
        gl::vertex( toVec3f( hand.palmPosition() ) + (toVec3f( hand.palmNormal() ) * Vec3f( lineLength, lineLength, lineLength )) );
        gl::end();

        // 移動して、回転して、描画
        gl::pushModelView();
        gl::translate( toVec3f( hand.palmPosition() ) );
        gl::rotate( Quatf(Vec3f( 0, 1, 0 ), Vec3f( hand.palmNormal().x, hand.palmNormal().y, hand.palmNormal().z ) ) );
  ...
  void renderFrameParameter(const Leap::Frame& frame)
  {
    ...
    for ( auto hand : frame.hands() ) {
      ...
      ss << "Hand Palm Direction : " << hand.direction().x << ", " 
                                     << hand.direction().y << ", "
                                     << hand.direction().z << "\n";
