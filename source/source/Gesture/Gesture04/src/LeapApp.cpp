...
public:
  ...
  {
    // 設定値を保存する
    mLeap.config().setFloat("Gesture.KeyTap.MinForwardVelocity", mMinDownVelocity);
    mLeap.config().setFloat("Gesture.KeyTap.HistorySeconds", mHistorySeconds);
    mLeap.config().setFloat("Gesture.KeyTap.MinDistance", mMinDistance );
    mLeap.config().save();
  ...
  std::list<Leap::KeyTapGesture> mGestureList;
