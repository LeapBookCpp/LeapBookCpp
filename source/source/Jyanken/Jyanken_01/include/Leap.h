...
namespace Leap {
  ...
  public:
     ...
     * base of the thumb's metacarpal bone. 
     *
     * Pass a member of this enumeration to Finger::jointPosition() to get the
     ...
     * joint of interest.
     * @returns The Vector containing the coordinates of the joint position.
     * @since 2.0
     */
  ...
  public:
    ...
    std::string getString(const std::string& key) const {
      const char* cstr = getCString(key);
      std::string str(cstr);
      deleteCString(cstr);
      return str;
    }
  ...
  private:
    LEAP_EXPORT const char* getCString(const std::string& key) const;
  };
