...
public:
  ...
  void updateLeapObject()
  {
    ...
    for ( auto hand : mCurrentFrame.hands() ) {
      if ( hand.isRight() ) {
        mRightHandMotion.update( hand, mLastFrame );
      }
      else {
        mLeftHandMotion.update( hand, mLastFrame );
      }
    }

    mFrameMotion.update( mCurrentFrame, mLastFrame );
  }
  ...
  void drawLeapObject()
  {
    ...
    // 表示処理
    mFrameMotion.draw( 0 );
    mRightHandMotion.draw( 150 );
    mLeftHandMotion.draw( -150 );
  ...
  Motion mFrameMotion;
  Motion mRightHandMotion;
  Motion mLeftHandMotion;
};
